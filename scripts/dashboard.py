...
def discover_samples(results_dir: Path) -> dict[str, dict[str, Path | None]]:
    ...
    for sample_dir in sorted(results_dir.iterdir()):
        ...
        sid = sample_dir.name
        slot_map = {
            "nanoplot":  _find_glob(sample_dir, "nanoplot/*.html"),
...
class LegionellaPipelineDashboard:
    ...
    def _render_elgato(self, data: dict):
        ...
        # Build hits dataframe - only include loci that actually have BLAST hits
        rows = []
        for locus, lhits in hits.items():
            if lhits:  # Check if hits exist for this locus
                for hit in lhits:
                    rows.append({
                        "locus":  locus,
                        "allele": hit[0],
        ...
        # Show which loci have BLAST hits vs MLST-only results
        blast_loci = set(hits.keys())
        mlst_loci = set(allele_cols)
        mlst_only = mlst_loci - blast_loci
        
        if mlst_only:
            st.info(f"MLST-only loci (no BLAST hits): {', '.join(sorted(mlst_only))}")
